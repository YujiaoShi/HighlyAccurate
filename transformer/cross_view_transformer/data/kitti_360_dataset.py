...
"""
TODO: 
1. For loading KITTI-360 images, no need for SingleTon! ?
2. For each Kitti360Dataset.__getitem__(self,idx)
   return a Sample() of information we need!
...
def get_data(
    ...
    **dataset_kwargs
    ...
    helper = Kitti360Singleton(dataset_dir, version)
    transform = SaveDataTransform(labels_dir)

    # Format the split name
    split = f'mini_{split}' if version == 'v1.0-mini' else split
    split_scenes = get_split(split, 'kitti-360')
    ...
    for scene_name, scene_record in helper.get_scenes():
        ...
        data = Kitti360Dataset(scene_name, scene_record, helper,
...
class Kitti360Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Get image information from Kitti dataset
        # Refer to the dataloader of Kitti360-dataset
